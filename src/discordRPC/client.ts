...
export class Client {
    ...
    private async authorize(args: RPCLoginOptions) {
        ...
        if (args.clientSecret && args.rpcToken) {
            const body = await this.fetch('POST', '/oauth2/token/rpc', new URLSearchParams({ client_id: this.clientID, client_secret: args.clientSecret }));
        ...
        const response = await this.fetch('POST', '/oauth2/token', new URLSearchParams({
            client_id: this.clientID,
    ...
    private parseDate(date?: Date): number | undefined {
        if (date) {
            ...
            if (val > 2147483647000)
                throw new RangeError('timestamp must fit into a unix timestamp');
            return val
        }
    }
